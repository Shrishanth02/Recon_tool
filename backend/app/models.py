...
class ExploitProposal(Base):
    ...
    *automated reconnaissance/validation* and *actual exploitation*. The engine
    ...
    #: The authorization envelope PINNED at approve time. Execution may not
    #: deviate from these: the target the approval authorized (resolved from the
    #: finding, never taken from the execute request), and the engine + mode the
    #: run may use. NULL on legacy rows / never-approved proposals — treated as
    #: the SAFE default at execute.
    approved_target: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    approved_engine: Mapped[str | None] = mapped_column(String(20), nullable=True)
    approved_mode: Mapped[str | None] = mapped_column(String(20), nullable=True)
    result: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
